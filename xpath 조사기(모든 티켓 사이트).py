...
from PyQt6.QtGui import QFont, QColor, QAction, QPalette, QIcon, QPixmap, QKeySequence
...
class XPathExplorer(QMainWindow):
    ...
    def _create_list_panel(self):
        ...
        self.table.setSelectionMode(QAbstractItemView.SelectionMode.SingleSelection) # TODO: [FEAT-002] MultiSelection
        self.table.setEditTriggers(QAbstractItemView.EditTrigger.NoEditTriggers)
        
        self.table.setColumnWidth(0, 30)  # 상태 아이콘
        self.table.setColumnWidth(1, 150) # 이름
        self.table.setColumnWidth(2, 100) # 카테고리
        self.table.setColumnWidth(4, 40)  # 삭제 버튼
    ...
    def _refresh_table(self, filter_cat=None):
        ...
        for item in items_to_show:
            ...
            # 숨겨진 데이터로 이름 저장 (식별용)
            self.table.item(row, 1).setData(Qt.ItemDataRole.UserRole, item.name)
    ...
    def _on_item_selected(self):
        ...
        row = selected[0].row()
        item_name = self.table.item(row, 1).data(Qt.ItemDataRole.UserRole)
